...
class OpenAIService:
    ...
    async def extract_user_details(self, user_query: str, previous_messages: list):
        try:
            system_prompt = """ 
                            Extract the latest user's details (name, email, phone number, address, visit date and time in UTC) from the given query and messages. 
            ...
            response = await self.get_gpt_response_with_history(
                prompt="Extract the user details", system_prompt=system_prompt, previous_messages=previous_messages
            )
            return response
        except Exception as e:
            return {
